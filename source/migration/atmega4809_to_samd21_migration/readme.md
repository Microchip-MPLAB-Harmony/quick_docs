...
<a id="Install-MCC"> </a>
### 1. Install MCC
- Install the MPLAB® Code Configurator (MCC) Plug-in in the MPLAB X IDE if not installed.
- Verify the MPLAB® Code Configurator (MCC) Plug-in is installed.
    ...
    - You will see **MPLAB® Code Configurator v5** in the menu.
        ...
        <img src = "images/mcc_lanuch.png" width="700" height="300" align="middle">

<a id="Create-MPLAB-Harmony-v3-Project"> </a>
### 2. Create MPLAB Harmony v3 Project

- Select **File > New Project** from the main IDE menu.

    <img src = "images/new_project_selection.png" width="400" height="400" align="middle">
...
- In the **Project Settings** window, apply the following settings:
    1. **Project Name**: enter the project’s logical name as "*atmega4809_to_sam_d21_cnano*". This is the name that will be shown from within the MPLAB X IDE.
    2. **Project Location**: indicates the path to the root folder of the new project. All project files will be placed inside this folder. The project location can be any valid path, for example: `<your project path>`/atmega4809_to_sam_d21_migration.
...
- Launch **Clock Easy View** by going to **Project Graph > Plugins** menu and then select **Clock Configuration**.
    ...
    <img src = "images/clock_launch.png" width="400" height="250" align="middle">
...
- Double click or drag and drop **SERCOM5** to add the USART Peripheral Library (PLIB) to the project graph.
    ...
    **Note:** SAM D21 Curiosity Nano On-Board Nano Debugger also includes a Virtual Com port which is routed to SERCOM5, therefore in this application, SERCOM5 is chosen.
    ...
    <img src = "images/selecting_sercom5.png" width="700" height="550" align="middle">
...
- Select the **SERCOM5 Peripheral Library** in the project graph. Verify that the default "SERCOM Operation Mode" configuration is set as USART and configure 9600 baud rate and then select **SERCOM PAD[2]** in Recieve Pinout and the standard 8-N-1 (eight data bits, no parity bit and one Stop bit) frame format.
    ...
    <img src = "images/sercom5_config.png" width="650" height="380" align="middle">

### 5. Configuring USART pins using MCC PIN Configurator
...
- Now, generate the code by using MCC. From the left side tab **Resource Management [MCC]**, go to **Project Resources** and then click on the **Generate** button.
    ...
    <img src = "images/generate_code.png" width="350" height="350" align="middle">
...
### 8. Build and Program
- Before proceeding to Build and program, set up the compiler toolchain. Click on the **Projects** tab on the top left pane in MPLAB X IDE. Right click on the project name **atmega4809_to_sam_d21_cnano** and go to **Properties**.

    <img src = "images/project_view.png" width="420" height="700" align="middle">

- Make sure that XC32 (v4.40) is selected as the Compiler Toolchain for XC32. Click on **Apply** and then click on **OK**.
...
### 10. Configure/Enable Formatted STDIO, USART Receive and SWITCH/LED functionality
...
- If you closed MCC accidently and would like to open it again, go to **Tools > Embedded > MPLAB® Code Configurator v5** in MPLAB X IDE.
...
- Select the MCC **Pin Settings** tab and then scroll down to the PORT pin **PB10** (Pin Number 19) in the **Pin ID** column and configure **PB10** as an output pin for LED functionality as shown below.
...
- Under the **Device Resources** tab, expand **Harmony > Peripheral > EIC**. Select and double click on **EIC** to add the EIC module to the project.
    ...
    <img src = "images/selecting_eic.png" width="750" height="600" align="middle">
...
- Configure the EIC block to generate an interrupt every time the user presses the switch SW0 as shown below and enable filter functionality to avoid electrical noise on the switch pin.
    ...
    <img src = "images/eic_settings1.png" width="550" height="500" align="middle">

- Enable Systick to create a delay for toggling LED0, click on **System** in **Project Graph** and Configure as below.

    <img src = "images/systick_settings.png" width="600" height="450" align="middle">

- Click on the **Generate** button to generate the code.
- Navigate to the **Projects** tab in MPLAB X IDE and select main.c, add below code snippet to receive the command from serial terminal and a function to control the LED0 on the SAM D21 Curiosity Nano Evaluation Kit.
...
### 11. Verify Formatted STDIO, USART Receive and SWITCH/LED functionality
...
### 12. Extend application to configure SPI loopback using DMA based Transfers
...
- Select the **Project Graph** tab. Under the **Device Resources** tab, expand **Peripheral > SERCOM**. Select and double click on **SERCOM1** to add the SPI module to the project.
...
- In **Project Graph** select **SERCOM1** and configure as below
    ...
    <img src = "images/spi_config.png" width="650" height="400" align="middle">

- Open **Pin Table**, Select **SERCOM1** and configure the pins as shown below

    <img src = "images/spi_pin.png" width="800" height="250" align="middle">

- Launch **DMA Configuration** from **Project Graph > Plugins > DMA Configuration**.

    <img src = "images/dma_launch.png" width="500" height="300" align="middle">
...
- Click on the **Generate** button to generate the code.
- In main.c, Add below code snippet
...
### 13. Verify extended functionality - SPI loopback
